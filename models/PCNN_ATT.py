...
class PCNN_ATT(BasicModule):
    ...
    def init_word_emb(self):
        ...
        tensor_unk = torch.randn((1, self.opt.word_dim))
        w2v = torch.cat((w2v, tensor_unk), dim=0)
        pos1_vec = [np.zeros(self.opt.pos_dim)]
        pos1_vec.extend([np.random.uniform(low=-1.0, high=1.0, size=self.opt.pos_dim) for _ in range(self.opt.limit * 2 + 1)])
        pos2_vec = [np.zeros(self.opt.pos_dim)]
        pos2_vec.extend([np.random.uniform(low=-1.0, high=1.0, size=self.opt.pos_dim) for _ in range(self.opt.limit * 2 + 1)])
        p1_2v = torch.from_numpy(np.array(pos1_vec, dtype=np.float32))
        p2_2v = torch.from_numpy(np.array(pos2_vec, dtype=np.float32))
    ...
    def get_bags_feature(self, bags):
        ...
        bags_feature = []
        for bag in bags:
            # print("len of bag:{}".format(len(bag)))
            # print(bag)
            if self.opt.use_gpu:
